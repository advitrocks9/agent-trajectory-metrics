...
A separate logistic regression on trajectory shape plus three patch-vs-gold similarity features predicts the resolved flag at LOMO mean AUC **0.775**, 95% bootstrap CI [0.742, 0.803] (clustered by `instance_id`). This number uses the gold SWE-bench patch as input to compute Jaccard overlap and Qwen / Mellum cosine to ground truth, so it is a diagnostic of how well patch-vs-gold similarity correlates with `resolved` in-distribution, not an in-flight classifier. The strongest single patch-content feature is mean-pooled Mellum-4B-sft-python cosine at 0.764, with the indentation-aware Jaccard at 0.758 (paired delta Mellum minus Jaccard +0.0044, CI [-0.021, +0.029], includes 0) and Qwen-Coder-1.5B at 0.737 (paired delta vs Mellum +0.031, CI [+0.016, +0.046], excludes 0). Mellum beats Qwen; a 30-line `(file, +/-, payload)` Jaccard baseline that respects indentation ties Mellum within the paired CI.

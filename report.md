...
**Caveats.**
* Patch embeddings are mean-pooled hidden states from `AutoModel`, which discards Mellum's `lm_head.weight` (logged as UNEXPECTED in `data/embed_mellum.log`). That is correct for embedding extraction but the SFT-specific specialisation in the LM head is lost. Mean-pool is also the wrong default for a causal LM: the conventional choice is the last non-pad token's hidden state, which has attended to the full sequence. `embed_mellum_lasttoken.py` is the script for that; the committed `embeddings_mellum.npy` is mean-pooled because that was the v1 default, so the Mellum row in the table above is the mean-pool number and a last-token rerun is the obvious next step. The committed `embeddings_mellum.npy` was also produced with the bare-diff input, no `<filename>` / `<fim_*>` formatting; that is out-of-distribution for an FIM-trained model. `embed_mellum.py` now defaults to `--input-format fim`. The numbers above are from the bare-diff mean-pool embeddings, so the Mellum row is a lower bound on what proper pooling and input formatting would give.

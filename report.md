...
**7. Test-pass fraction as a feature, on a 50-instance subset (queued, not run in this revision).** I picked 50 instances stratified across 0/1/2/3/4-of-4-models-resolve buckets (`data/exec_subset.json`) and built per-model prediction JSONLs (`data/exec_predictions/*.jsonl`) for the SWE-bench Docker harness, with a driver in `run_execution.py predictions | harness | merge`. The new feature is `tests_pass_frac = (FAIL_TO_PASS.success + PASS_TO_PASS.success) / total`. Plan: re-run `classify_compare.py` with a `+execution` subset under instance-grouped paired bootstrap on the AUC delta, on the 200-row subset only. The harness run didn't finish here: docker-desktop returned `containerd metadata.db: input/output error` on first image pull because the Mac was at 98% disk after I cached Mellum-4B. The scaffolding is in place; numbers go into `data/execution.csv` once a runner with ~50 GB free is available. The point of the experiment: if `tests_pass_frac` dominates the LOMO AUC, the diff-shape and embedding features add little once the tests run; before they run, Jaccard is the cheapest sufficient signal.

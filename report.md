...
The five top SWE-bench Verified models score within 4.2 points on pass@1 (76.8 -> 72.8); their per-instance cost spans 10x. A logistic regression on trajectory shape plus patch-content features predicts the resolved flag at LOMO mean AUC 0.78 (95% bootstrap CI [0.75, 0.80]). The strongest single patch-content feature is, surprisingly, the Jaccard overlap of changed lines between predicted and ground-truth diff: a paired bootstrap on instance_id puts it within +/-0.025 of mean-pooled JetBrains/Mellum-4B (delta -0.001, CI [-0.025, +0.023]). Mellum does beat Qwen-Coder-1.5B in the same paired sense (+0.031, CI [+0.016, +0.046] excludes 0), but a 30-line Jaccard baseline closes the gap. Pass@1 is saturated at the top of the leaderboard; the trajectory is not.
...
**3. Patch-content features dominate; encoder choice barely matters once Jaccard is in.** L2-regularised LR with leave-one-model-out across the four labelled models, bootstrap CIs over pooled out-of-fold predictions:
...
**7. Killer experiment: tests-pass fraction on a 50-instance subset (queued, not run in this revision).** I picked 50 instances stratified across 0/1/2/3/4-of-4-models-resolve buckets (`data/exec_subset.json`) and built per-model prediction JSONLs (`data/exec_predictions/*.jsonl`) for the SWE-bench Docker harness, with a driver in `run_execution.py predictions | harness | merge`. The new feature is `tests_pass_frac = (FAIL_TO_PASS.success + PASS_TO_PASS.success) / total`. The plan: re-run `classify_compare.py` with a `+execution` subset under instance-grouped paired bootstrap on the AUC delta, on the 200-row killer subset only. The actual harness run did not complete in this revision: the local docker-desktop instance returned `containerd metadata.db: input/output error` on first image pull because the Mac was at 98% disk after I cached Mellum-4B. The pipeline scaffolding is in place and the harness commands are documented in `run_execution.py`; numbers go into `data/execution.csv` once a runner with ~50 GB free is available. This is the load-bearing experiment for the report's framing -- if `tests_pass_frac` dominates the LOMO AUC, the diff-shape and embedding features add little once you can run the tests; before you can run them, Jaccard remains the cheapest sufficient signal.
